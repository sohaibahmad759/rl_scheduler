...
def get_args():
    ...
    ap.add_argument('--out', '-o', required=False, default='../../twitter/asplos/zipf_flat_uniform',
...
def main(args):
    ...
    with open(input_file, mode='r') as rf:
        ...
        for model in range(num_models):
            ...
            arrivals = (arrivals / sum(arrivals) * seconds * 1000).astype(int)

            print(f'model: {model}, arrivals: {arrivals}, sum: {sum(arrivals)}, '
                  f'requests for model: {requests_for_model}')
            ...
            for time in arrivals:
                current += time
                uniform_time = time
                traces[model].append(current)
            
            while current < seconds * 1000:
                traces[model].append(current)
                current += uniform_time
